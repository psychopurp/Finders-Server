...
`user_phone` varchar(30) NOT NULL COMMENT '手机号',
`user_password` varchar(30) NOT NULL COMMENT '密码',
`user_nickname` varchar(30) NOT NULL COMMENT '昵称',
...
PRIMARY KEY (`user_id`) ,
UNIQUE INDEX `unique_phone` (`user_phone` ASC)
);
CREATE TABLE `user_infos` (
`user_id` varchar(30) NOT NULL COMMENT '用户ID',
`userinfo_truename` varchar(40) NULL COMMENT '真实姓名',
`userinfo_address` varchar(200) NULL COMMENT '所在地',
`userinfo_sex` varchar(4) NULL COMMENT '性别',
`userinfo_sexual` varchar(8) NULL COMMENT '性取向',
`userinfo_feeling` varchar(20) NULL COMMENT '感情状况',
`userinfo_birthday` varchar(20) NULL COMMENT '生日',
`userinfo_intro` varchar(400) NULL COMMENT '简介',
`userinfo_bloodtype` varchar(8) NULL COMMENT '血型',
`userinfo_eamil` varchar(60) NULL COMMENT '邮箱',
`userinfo_qq` varchar(30) NULL COMMENT 'QQ',
`userinfo_wechat` varchar(30) NULL COMMENT '微信',
`userinfo_profession` varchar(60) NULL COMMENT '职业信息',
`userinfo_school` varchar(30) NULL COMMENT '学校',
`userinfo_constellation` varchar(40) NULL COMMENT '星座',
`created_at` datetime NOT NULL ON UPDATE CURRENT_TIMESTAMP COMMENT '创建时间',
`updated_at` datetime NULL ON UPDATE CURRENT_TIMESTAMP COMMENT '更新时间',
`userinfo_credit` int NOT NULL COMMENT '用户信誉积分',
`userinfo_tag` text CHARACTER SET 用户标签 NULL,
PRIMARY KEY (`user_id`) 
);
CREATE TABLE `admins` (
...
`community_description` text CHARACTER SET 社区简介 NULL,
`community_status` int NOT NULL COMMENT '社区状态',
`updated_at` datetime NULL ON UPDATE CURRENT_TIMESTAMP COMMENT '更新时间',
`background` varchar(200) NULL COMMENT '背景',
`board` text CHARACTER SET 公告板 NULL,
`deleted_at` datetime NULL ON UPDATE CURRENT_TIMESTAMP COMMENT '删除时间',
...
`activity_info` text CHARACTER SET 帖子内容 NULL,
`created_at` datetime NOT NULL ON UPDATE CURRENT_TIMESTAMP COMMENT '创建时间',
`collect_num` int NOT NULL COMMENT '收藏次数',
`comment_num` int NOT NULL COMMENT '评论次数',
`read_num` int NOT NULL COMMENT '阅读次数',
`activity_tag` text NULL COMMENT '帖子标签',
`picture_id` varchar(30) NULL COMMENT '帖子图片ID',
`user_id` varchar(30) NULL COMMENT '发表用户ID',
...
`id` int NOT NULL,
`user_id` varchar(30) NULL COMMENT '社区用户ID',
